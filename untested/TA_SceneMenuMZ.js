 ...
 * @author Tamaki Awana
 * @help メニュー画面のレイアウトを変更します。
 * 主な変更点は以下の通りとなります。
 * 
 * ・各キャラクターの背景に、ベタ塗りの領域か、
 * 　独自のスキンを表示する機能を搭載しています。
 ...
 * @min 1
 * @max 9007
 * @desc メニューステータスウィンドウに一度に表示するメンバーの数です。
 * @default 4
 * @parent MenuStatusWindow
 * 
 * @param ReserveMemberColor
 * @desc 控え状態のメンバーに重ねる色をRGBAで設定します。
 * @default 0, 0, 0, 0.4
 * @parent MenuStatusWindow
 * 
...
(function () {
  ...
  const menubg = String(parameters["SceneMenuBackground"] || "");
  const menufg = String(parameters["SceneMenuForeground"] || "");

  const mcwx = Number(parameters["MCWindowX"] || 0);
  const mcwy = Number(parameters["MCWindowY"] || 96);
  const mcwwidth = Number(parameters["MCWindowWidth"] || 180);
  const mcwheight = Number(parameters["MCWindowHeight"] || 520);
  const mcwop = Number(parameters["MCWindowOp"] || 0);

  const menustatusx = Number(parameters["MenuStatusWindowX"] || 180);
  const menustatusy = Number(parameters["MenuStatusWindowY"] || 72);
  const menustatuswidth = Number(parameters["MenuStatusWindowWidth"] || 636);
  const menustatusheight = Number(parameters["MenuStatusWindowHeight"] || 552);
  const menustatusop = Number(parameters["MenuStatusWindowOp"] || 255);

  const stmems = Number(parameters["StatusMembers"] || 4);

  const rsvmcolor = String(parameters["ReserveMemberColor"] || "0, 0, 0, 0.4");

  const mindexbg = String(parameters["MenuIndexBackground"] || "filled");
  const mindexcolor = String(parameters["MenuIndexColor"] || "0, 0, 0, 0.4");
  const mindexskin = String(parameters["MenuIndexSkin"]);

  const mpwx = Number(parameters["MPWindowX"] || 180);
  const mpwy = Number(parameters["MPWindowY"] || 0);
  const mpwwidth = Number(parameters["MPWindowWidth"] || 444);
  const mpwheight = Number(parameters["MPWindowHeight"] || 72);
  const mpwop = Number(parameters["MPWindowOp"] || 255);
  ...
  //Window_MenuStatus
  Window_MenuStatus.prototype.ReserveColor = function () {
    const color = "rgba(" + rsvmcolor + ")";
    return color;
  };
  ...
  Window_MenuStatus.prototype.windowWidth = function () {
    return Graphics.boxWidth - menustatuswidth;
  };
  ...
  Window_MenuStatus.prototype.drawItemBackground = function (index) {
    ...
    if (!actor.isBattleMember()) {
      this.contents.bitmap.fillRect(1, 1, w, h, this.ReserveColor());
    }
  ...
  Window_MenuStatus.prototype.drawItemImage = function (index) {
    var actor = $gameParty.members()[index];
    var rect = this.itemRect(index);
    var yfix = rect.height / 2 - this.lineHeight();
    this.changePaintOpacity(actor.isBattleMember());
    this.drawActorFace(actor, rect.x + 1, rect.y + yfix - 6, 144, 82);
  ...
  Window_MenuStatus.prototype.drawActorSimpleStatus = function (actor, x, y, width) {
    ...
    this.drawActorIcons(actor, x + 96, y + lineHeight * 1);
  ...
  Window_MenuStatus.prototype.drawItemStatus = function (index) {
    ...
    var rect = this.itemRect(index);
    var x = rect.x + 162;
    var y = rect.y + rect.height / 2 - this.lineHeight();
