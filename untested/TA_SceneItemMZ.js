...
(() => {
  ...
  const itembg = String(parameters["SceneItemBackground"] || "");
  const itemfg = String(parameters["SceneItemForeground"] || "");

  const itemcwx = Number(parameters["ItemCategoryWindowX"] || 0);
  const itemcwy = Number(parameters["ItemCategoryWindowY"] || 108);
  const itemcwwidth = Number(parameters["ItemCategoryWindowWidth"] || 180);
  const itemcwheight = Number(parameters["ItemCategoryWindowHeight"] || 420);
  const itemcwop = Number(parameters["ItemCategoryWindowOp"] || 255);
  ...
  const keyname_item = String(parameters["KeyItemName"]);

  const itemlistx = Number(parameters["ItemListWindowX"] || 180);
  const itemlisty = Number(parameters["ItemListWindowY"] || 108);
  const itemlistwidth = Number(parameters["ItemListWindowWidth"] || 696);
  const itemlistheight = Number(parameters["ItemListWindowHeight"] || 516);
  const itemlistop = Number(parameters["ItemListWindowOp"] || 255);

  const itemlistcl = Number(parameters["ItemListWindowColumns"] || 2);
  ...
  Scene_Item.prototype.createItemWindow = function () {
    const wx = itemlistx;
    const wy = itemlisty;
    const ww = itemlistwidth;
    const wh = itemlistheight;
