...
(() => {
  ...
  const nwxoffs = Number(parameters["NameWindowXOffset"] || 0);
  const nwyoffs = Number(parameters["NameWindowYOffset"] || 0);
  const nwwidth = Number(parameters["NameWindowWidth"] || 192);
  const nwheight = Number(parameters["NameWindowHeight"] || 72);
  const nwop = Number(parameters["NameWindowOp"] || 255);
  const nwbg = String(parameters["NameWindowBG"]);
  const namefs = Number(parameters["NameFontSize"] || 28);
  const nwtxtx = Number(parameters["NameTextX"] || 0);
  const nwtxty = Number(parameters["NameTextY"] || 0);
  const nameali = String(parameters["NameTextAlign"] || "left");
  const nwopac = (parameters["NameWindowOpenAction"] || true);
  ...
  Window_ACName.prototype.refresh = function () {
    ...
    this.contents.clear();
    if (nwbg) {
      const bitmap = ImageManager.loadSystem(nwbg);
