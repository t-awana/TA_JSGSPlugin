 ...
 * 
 * 【プラグインコマンド】
 * TA_ResultSceneHorz SetGold [獲得金額]
 * 獲得金額の数値を設定します。
 * 
 * TA_ResultSceneHorz SetExp [獲得経験値]
 * 獲得経験値の値を設定します。
 * 
 * TA_ResultSceneHorz SetAP [獲得AP]
 * TA_AbilityPoint導入時に獲得できるAPの値を設定します。
 * 
 * TA_ResultSceneHorz SetPreCommon [コモンイベントのID]
...
(function () {
  ...
  var crwwidth = Number(parameters["CharacterResultWindowWidth"] || 408);
  var crwheight = Number(parameters["CharacterResultWindowHeight"] || 552);
  var lnst = String(parameters["LearnNewSkillText"]);

  var imwx = Number(parameters["ItemResultWindowX"] || 408);
  var imwy = Number(parameters["ItemResultWindowY"] || 72);
  var imwwidth = Number(parameters["ItemResultWindowWidth"] || 408);
  var imwheight = Number(parameters["ItemResultWindowHeight"] || 480);
  ...
  var _Scene_ResultHorz_createBackground = Scene_ResultHorz.prototype.createBackground;
