 ...
 * @desc 設定するAPの値です。
 * 
 * @command ResultVertSetPreCommonEvent
...
(() => {
  ...
  const crwwidth = Number(parameters["CharacterResultWindowWidth"] || 816);
  const crwheight = Number(parameters["CharacterResultWindowHeight"] || 408);
  const lnst = String(parameters["LearnNewSkillText"]);

  const imwx = Number(parameters["ItemResultWindowX"] || 0);
  const imwy = Number(parameters["ItemResultWindowY"] || 480);
  const imwwidth = Number(parameters["ItemResultWindowWidth"] || 576);
  const imwheight = Number(parameters["ItemResultWindowHeight"] || 144);
 ...
 //Plugin Commands
  PluginManager.registerCommand(pluginName, "ResultVertSetGold", args) => {
    const gold = Number(args.SetGold);
    $gameSystem.Result_SetGold(gold);
  });

  PluginManager.registerCommand(pluginName, "ResultVertSetExp", args) => {
    const exp = Number(args.SetExp);
    $gameSystem.Result_SetExp(exp);
  });

  PluginManager.registerCommand(pluginName, "ResultVertSetAP", args) => {
    const ap = Number(args.SetAP);
    $gameSystem.Result_SetAP(ap);
  });

  PluginManager.registerCommand(pluginName, "ResultVertSetPreCommonEvent", args) => {
    const precommonev = Number(args.SetPreCommon);
  });

  PluginManager.registerCommand(pluginName, "SceneResultVertOpen", args => {
     SceneManager.push(Scene_ResultVert);
  });
  ...
  const _Scene_ResultVert_createBackground = Scene_ResultVert.prototype.createBackground;
